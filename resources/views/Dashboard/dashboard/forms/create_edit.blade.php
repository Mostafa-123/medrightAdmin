    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function createFormExisitingRow(row) {
            ...
            if (inputTypeValue == 'number' || inputTypeValue == 'phone' || inputTypeValue == 'password' ||
                ...
                labelTd.className = "col-1";
                const label = document.createElement("label");
                label.innerText = row['input_type'] === 'textarea' ? 'Text Area' : inputTypeValue.charAt(0).toUpperCase() + inputTypeValue.slice(1);
                ...
                tr.appendChild(createElement("col-3", "text", "placeholder[]", "Placeholder", row[
                        'placeholder'],
                    "placeholderid", true));
                tr.appendChild(createElement("col-2", "number", "length[]", "Length", row['length'], "lengthid",
                    true));
                ...
                filesNumInput.display=row['multi_file'] == 1 ? true : false;
                filesNumInput.required = row['required'] ? true : false;
                ...
                tr.appendChild(createElement("col-3", "text", "fieldName[]", "Name", row['name'], "nameid",
                    true));
                tr.appendChild(createElement("col-2", "number", "fileSize[]", "File Size", row['file_size'],
                    "fileSizeid", true));
                ...
                tr.appendChild(createElement("col-1", "checkbox", "required[]", "input", row['required'], "requiredid", false, "Required"));
        ...
        function createRow() {
            ...
            if (inputTypeValue == 'number' || inputTypeValue == 'phone' || inputTypeValue == 'password' ||
                ...
                labelTd.className = "col-1";
                const label = document.createElement("label");
                label.innerText = inputTypeValue === 'textarea' ? 'Text Area' : inputTypeValue.charAt(0).toUpperCase() + inputTypeValue.slice(1);
                labelTd.appendChild(label);
                tr.appendChild(labelTd);
                const actionTd = document.createElement("div");
                actionTd.className = "col-1";
                ...
                tr.appendChild(createElement("col-3", "text", "placeholder[]", "Placeholder", null,
                    "placeholderid",
                    true));
                tr.appendChild(createElement("col-2", "number", "length[]", "Length", null, "lengthid", true));
                ...
                tr.appendChild(createElement("col-2", "checkbox", "required[]", "input", null, "requiredid",
                    false,
                    "Required"));
                const numOptionsInput = createInputField("New Options");
                numOptionsInput.type = "number";
                numOptionsInput.min = "0";
                numOptionsInput.name = "numOptions[]";
                numOptionsInput.id = "numOptions";
        ...
        function createOptionsInputs(parentElement, field = null) {
            ...
            let optionsContainer = field ? parentElement : parentElement.querySelector('#optionsContainer');
            ...
            for (let i = 1; i <= numOptions; i++) {
                ...
                optionInput.className="col-10";
                const removeButton = document.createElement("button");
                removeButton.innerText = "X";
                removeButton.className = "btn btn-danger remove-option col-2";
                removeButton.addEventListener("click", () => {
                ...
                if (field && field['files_type']) {
                    if (field['files_type'][i - 1]) {
                        filesType = Array.isArray(field['files_type']) ? field['files_type'] : JSON.parse(field['files_type']);
        ...
        function updateRowsData() {
            ...
            rows.forEach((row) => {
                ...
                if (inputTypeInput) {
                    ...
                    if (["text", "number", "phone", "textarea", "password"].includes(inputTypeValue)) {
                        const inputTypeInput = row.querySelector('input[name="input_type[]"]');
